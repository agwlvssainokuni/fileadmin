...
describe FileAdmin::FetchFile do
  ...
  let(:base_conf) { {
      "label" => "リモート退避試験",
      "basedir" => "#{Dir.pwd}/testdir/work",
      "host" => "localhost",
      "rdir" => "#{Dir.pwd}/testdir/src/",
  ...
  describe "valid?" do
    ...
    context "extなし" do
      let(:conf) { base_conf.merge("ext" => nil) }
      it { expect(subject).not_to be_valid }
    end
  ...
  describe "process" do
    ...
    describe "正常系" do
      ...
      shared_examples_for "退避して正常終了" do
        ...
        context "通常 (retval,src,work,dest)" do
          let(:dry_run) { false }
          it { expect(@retval).to be_truthy }
          it {
            files_in_process.each {|f|
              expect(Pathname("testdir/src/#{f}")).not_to exist
              expect(Pathname("testdir/src/#{f}.done")).to be_file
            }
          }
          it {
            files_not_in_process.each {|f|
              expect(Pathname("testdir/src/#{f}")).to be_file
              expect(Pathname("testdir/src/#{f}.done")).not_to exist
            }
          }
          it {
            file_list.each {|f|
              expect(Pathname("testdir/work/#{f}")).not_to exist
              expect(Pathname("testdir/work/#{f}.done")).not_to exist
            }
          }
          it {
            files_in_process.each {|f|
              expect(Pathname("testdir/dest/#{f}")).to be_file
              expect(Pathname("testdir/dest/#{f}.done")).not_to exist
            }
          }
          it {
            files_not_in_process.each {|f|
    ...
    describe "境界系" do
      ...
      shared_examples_for "退避しないで正常終了" do
        ...
        context "ドライ (retval,src,work,dest)" do
          let(:dry_run) { true }
          it { expect(@retval).to be_truthy }
          it {
            file_list.each {|f|
    ...
    describe "異常系" do
      ...
      describe "basedir不正" do
        ...
        context "ドライ (retval,src,work,dest); dryでもchdirする" do
          let(:dry_run) { true }
      ...
      describe "移動失敗 (書込権限なし)" do
        ...
        context "通常 (retval,src,work,dest)" do
          ...
          it {
            file_list.each {|f|
              expect(Pathname("testdir/work/#{f}")).to be_file
            }
          }
          it {
            file_list.each {|f|
